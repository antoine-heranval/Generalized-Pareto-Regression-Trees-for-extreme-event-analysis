...
simulate_data_burr=function(n=100,seed=1,sigma_tan_h=20){
  ...
  X <- t*X1+(1-t)*X2
  if(sigma_tan_h<+Inf){
    Y <- rburr(n = 10*n, shape1=1,shape2 = 1/(1 + 0.25*tanh(sigma_tan_h*(X-0.25)) + 0.25*tanh(sigma_tan_h*(X-0.75))),scale = 1/10 )
  }else{
    Y <- rburr(n = 10*n, shape1=1,shape2 = 1/(1 + 0.25*step_simulate_data(X-0.25) + 0.25*step_simulate_data(X-0.75)),scale = 1/10)
  }
